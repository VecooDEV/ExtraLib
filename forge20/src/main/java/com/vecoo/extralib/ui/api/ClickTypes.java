...
public enum ClickTypes {
    ...
    ClickTypes(boolean isLeft, boolean isRight, boolean isMiddle, boolean shift, int value, boolean numKey, boolean isDragging) {
        this.isLeft = isLeft;
    ...
    public static ClickTypes toClickType(ClickType action, int button, int slot) {
        switch (action) {
            case PICKUP -> {
            ...
            case SWAP -> {
                if (button >= 0 && button < 9) {
                    return ClickTypes.values()[button + 4];
                } else if (button == 40) {
                    return ClickTypes.OFFHAND_SWAP;
                }
            }
