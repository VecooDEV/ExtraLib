...
public class UtilTask {
    private Consumer<UtilTask> consumer;
    ...
    private UtilTask(Consumer<UtilTask> consumer, long delay, long interval, long iterations) {
    ...
    public static class Builder {
        ...
        public UtilTask build() {
            if (consumer == null) {
                throw new IllegalStateException("consumer must be set");
            }
            UtilTask task = new UtilTask(consumer, delay, interval, iterations);
            TaskTickHandler.addTask(task);
            return task;
        }
    }
}