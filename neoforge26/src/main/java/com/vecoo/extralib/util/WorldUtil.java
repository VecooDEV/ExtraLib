...
public final class WorldUtil {
    ...
    public static ServerLevel findLevelByName(@NotNull String levelName) {
        for (ServerLevel level : ExtraLib.getInstance().getServer().getAllLevels()) {
            if (level.dimension().registry().getPath().equals(levelName.toLowerCase(Locale.ROOT))) {
                return level;
            }
        }
    ...
    public static AABB getWorldBounds(@NotNull Level level) {
        ...
        return new AABB(worldBorder.getMinX(), level.getMinY(), worldBorder.getMinZ(),
                worldBorder.getMaxX(), level.getMaxY(), worldBorder.getMaxZ());
    }
}