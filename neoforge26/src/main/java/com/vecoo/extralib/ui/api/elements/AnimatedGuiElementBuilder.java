...
public class AnimatedGuiElementBuilder implements GuiElementBuilderInterface<AnimatedGuiElementBuilder> {
    ...
    @NotNull
    public AnimatedGuiElementBuilder setSkullOwner(@NotNull GameProfile gameProfile) {
        if (!gameProfile.properties().isEmpty()) {
            return setSkullOwner(ResolvableProfile.createResolved(gameProfile));
        }

        if (gameProfile.name().isEmpty()) {
            return setSkullOwner(ResolvableProfile.createUnresolved(gameProfile.id()));
        }
        if (gameProfile.id().equals(Util.NIL_UUID)) {
            return setSkullOwner(ResolvableProfile.createUnresolved(gameProfile.name()));
        }

        return this;
    }

    public AnimatedGuiElementBuilder setSkullOwner(@NotNull String playerNAME) {
        return setSkullOwner(ResolvableProfile.createUnresolved(playerNAME));
    }

    public AnimatedGuiElementBuilder setSkullOwner(@NotNull UUID playerUUID) {
        return setSkullOwner(ResolvableProfile.createUnresolved(playerUUID));
    }

    public AnimatedGuiElementBuilder setSkullOwner(ResolvableProfile component) {
